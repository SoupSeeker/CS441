...
class PosSession(models.Model):
    ...
    def create(self, values):
        ...
        if self.user_has_groups('nemo_pos.group_pos_user'):
    ...
    def _validate_session(self, balancing_account=False, amount_to_balance=0, bank_payment_method_diffs=None):
        bank_payment_method_diffs = bank_payment_method_diffs or {}
        self.ensure_one()
        sudo = self.user_has_groups('nemo_pos.group_pos_user')
    ...
    def get_closing_control_data(self):
        ...
        return {
            ...
            'is_manager': self.user_has_groups("nemo_pos.group_pos_manager"),
    ...
    def action_view_order(self):
        return {
            ...
            'view_mode': 'tree,form',
            'views': [
                (self.env.ref('nemo_pos.view_pos_order_tree_no_session_id').id, 'tree'),
                (self.env.ref('nemo_pos.view_pos_pos_form').id, 'form'),
                ],
            'type': 'ir.actions.act_window',
        ...
        for session in sessions:
            if self.env['mail.activity'].search_count([('res_id', '=', session.id), ('res_model', '=', 'pos.session')]) == 0:
                session.activity_schedule(
                    'nemo_pos.mail_activity_old_session',
