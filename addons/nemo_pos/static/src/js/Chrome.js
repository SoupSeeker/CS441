odoo.define('nemo_pos.Chrome', function(require) {
    'use strict';
    ...
    const NumberBuffer = require('nemo_pos.NumberBuffer');
    const PopupControllerMixin = require('nemo_pos.PopupControllerMixin');
    const Registries = require('nemo_pos.Registries');
    const IndependentToOrderScreen = require('nemo_pos.IndependentToOrderScreen');
    const contexts = require('nemo_pos.PosContext');
    const { identifyError, posbus } = require('nemo_pos.utils');
    ...
    const models = require('nemo_pos.models');
    ...
    class Chrome extends PopupControllerMixin(PosComponent) {
        ...
        async _closePos() {
            ...
            if (!this.env.pos || this.env.pos.db.get_orders().length === 0) {
                window.location = '/web#action=nemo_pos.action_client_pos_menu';
            }
        ...
        _onPlaySound({ detail: name }) {
            let src;
            if (name === 'error') {
                src = "/nemo_pos/static/src/sounds/error.wav";
            } else if (name === 'bell') {
                src = "/nemo_pos/static/src/sounds/bell.wav";
            }
        ...
        _preloadImages() {
            ...
            for (let imageName of staticImages) {
                const image = new Image();
                image.src = `/nemo_pos/static/src/img/${imageName}`;
            }
        }
        ...
        _buildChrome() {
            if ($.browser.chrome) {
                ...
                if (parseInt(chrome_version, 10) >= 50) {
                    loadCSS('/nemo_pos/static/src/css/chrome50.css');
                }
            }
