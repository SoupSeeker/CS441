odoo.define('nemo_pos.PaymentScreen', function (require) {
    'use strict';

    const { parse } = require('web.field_utils');
    const PosComponent = require('nemo_pos.PosComponent');
    const { useErrorHandlers, useAsyncLockedMethod } = require('nemo_pos.custom_hooks');
    const NumberBuffer = require('nemo_pos.NumberBuffer');
    const { useListener } = require('web.custom_hooks');
    const Registries = require('nemo_pos.Registries');
    const { onChangeOrder } = require('nemo_pos.custom_hooks');
    const { isConnectionError } = require('nemo_pos.utils');
