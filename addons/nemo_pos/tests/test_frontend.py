...
class TestPointOfSaleHttpCommon(AccountTestInvoicingHttpCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        env = cls.env
        cls.env.user.groups_id += env.ref('nemo_pos.group_pos_manager')
        ...
        discount = env.ref('nemo_pos.product_product_consumable')
        cls.tip = env.ref('nemo_pos.product_product_tip')
        (all_pos_product - discount - cls.tip)._write({'active': False})

        # In DESKS categ: Desk Pad
        pos_categ_desks = env.ref('nemo_pos.pos_category_desks')

        # In DESKS categ: Whiteboard Pen
        pos_categ_misc = env.ref('nemo_pos.pos_category_miscellaneous')

        # In CHAIR categ: Letter Tray
        pos_categ_chairs = env.ref('nemo_pos.pos_category_chairs')
        ...
        env['product.pricelist'].create({
            ...
            'item_ids': [(0, 0, {
                ...
                'applied_on': '0_product_variant',
                'min_quantity': 2,
                'product_id': env.ref('nemo_pos.product_product_consumable').id,
            })],
        })
...
class TestUi(TestPointOfSaleHttpCommon):
    def test_01_pos_basic_order(self):
        ...
        self.env['ir.module.module'].search([('name', '=', 'nemo_pos')], limit=1).state = 'installed'
