...
function connect () {
	...
	if [ ${TIMEOUT_RETURN} -eq 124 ] && [ -z "${NO_AP}" ] ; then
		...
		sudo /home/pi/odoo/addons/nemo_pos/tools/posbox/configuration/wireless_ap.sh "force" &
	else
		...
		/home/pi/odoo/addons/nemo_pos/tools/posbox/configuration/keep_wifi_alive.sh &
	fi
