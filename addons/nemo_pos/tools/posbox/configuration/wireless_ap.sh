...
if [ -z "${WIRED_IP}" ] ; then
	...
	if [ -f "${WIFI_NETWORK_FILE}" ] && [ -z "${FORCE_HOST_AP}" ] ; then
		...
		/home/pi/odoo/addons/nemo_pos/tools/posbox/configuration/connect_to_wifi.sh &
	else
		...
		nginx -c /home/pi/odoo/addons/nemo_pos/tools/posbox/configuration/nginx_ap.conf
